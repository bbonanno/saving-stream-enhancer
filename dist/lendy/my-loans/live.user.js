...
// @version        0.6
// @timestamp      2017-04-18T16:06:51.457Z
   ...
   */
    ...
    */
     ...
     */
					setHeadersCss: function setHeadersCss(c) {
						...
						c.$headers.add($('thead ' + c.namespace + '_extra_headers')).removeClass(css.join(' ')).addClass(none).attr('aria-sort', 'none').find('.' + ts.css.icon).removeClass(cssIcon.join(' ')).addClass(cssIcon[2]);
						for (indx = 0; indx < len; indx++) {
     ...
     */
					...
					multisort: function multisort(c) {
						...
						if ((typeof textSorter === "undefined" ? "undefined" : _typeof(textSorter)) === 'object') {
							colMax = c.columns;
							while (colMax--) {
					...
					sortNatural: function sortNatural(a, b) {
						...
						for (indx = 0; indx < max; indx++) {
							...
							if ((typeof aFloat === "undefined" ? "undefined" : _typeof(aFloat)) !== (typeof bFloat === "undefined" ? "undefined" : _typeof(bFloat))) {
								aFloat += '';
								bFloat += '';
							}
							if (aFloat < bFloat) {
     ...
     */
					...
					characterEquivalents: {
						'a': "\xE1\xE0\xE2\xE3\xE4\u0105\xE5", // áàâãäąå
    ...
    */
			...
			;(function ($) {
				...
				ts.addWidget({
					...
					remove: function remove(table, c, wo, refreshing) {
						var tbodyIndex,
						    ...
						    $table = c.$table,
						    $tbodies = c.$tbodies,
						    events = 'addRows updateCell update updateRows updateComplete appendCache filterReset filterAndSortReset filterEnd search '.split(' ').join(c.namespace + 'filter ');
				...
				tsf = ts.filter = {
					...
					findRows: function findRows(table, filters, currentFilters) {
						if (table.config.lastSearch.join(',') === (currentFilters || []).join(',') || !table.config.widgetOptions.filter_initialized) {
							return;
						}
						var len,
						    norm_rows,
					...
					processOptions: function processOptions(table, column, arry) {
						...
						if (validColumn && c.$headerIndexed[column].hasClass('filter-select-nosort')) {
							...
							parsed.sort(function (a, b) {
								...
								} else if (validColumn && (typeof cts === "undefined" ? "undefined" : _typeof(cts)) === 'object' && cts.hasOwnProperty(column)) {
									// custom text sorter for a SPECIFIC COLUMN
					...
					format: function format(table, c, wo) {
						...
						$stickyTable = wo.$sticky = $table.clone().addClass('containsStickyHeaders ' + ts.css.sticky + ' ' + wo.stickyHeaders + ' ' + c.namespace.slice(1) + '_extra_table').wrap('<div class="' + ts.css.stickyWrap + '">'),
							...
							xWindow = $.isWindow($xScroll[0]),
							    attachTop = $attach.length ? yWindow ? $yScroll.scrollTop() : $yScroll.offset().top : $yScroll.scrollTop(),
			...
			;(function ($, window) {
				...
				$(function () {
					var s = '<style>' + 'body.' + ts.css.resizableNoSelect + ' { -ms-user-select: none; -moz-user-select: -moz-none;' + '-khtml-user-select: none; -webkit-user-select: none; user-select: none; }' + '.' + ts.css.resizableContainer + ' { position: relative; height: 1px; }' +
	...
	}, { "../../common.js": 3, "jquery": 1, "tablesorter": 2 }] }, {}, [4]);